...
const updateOrderItemFinancial = async ({
    orderReference,
    partName,
    field,
    value,
    ...
    const storedValue = field === 'fob_cost'
        ? Number(storedItem.fob_cost ?? storedItem.cost_fob)
        : Number(storedItem.sale_price ?? storedItem.price);

    if (!Number.isFinite(storedValue) || Math.abs(storedValue - value) > 0.001) {
        return {
            ok: false,
    ...
    return {
        ok: true,
        message: `${successLabel} actualizado a $${value.toFixed(2)} en ${resolved.order.readable_id || resolved.order.id}`,
        data: {
            orderId: resolved.order.id,
...
const setOrderFobCommand = {
    ...
    execute: (args, context) => updateOrderItemFinancial({
