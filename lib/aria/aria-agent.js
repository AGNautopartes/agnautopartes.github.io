...
export const requestAriaDecision = async ({
    ...
    const toolCalls = Array.isArray(modelMessage.tool_calls)
        ? modelMessage.tool_calls.filter(toolCall => toolCall?.function?.name)
        : [];
    if (toolCalls.length > 0) {
        const normalizedToolCalls = toolCalls.map((toolCall, index) => ({
            ...toolCall,
            id: toolCall.id || `aria_call_${index}`
        }));
        const commands = normalizedToolCalls.map(toolCall => ({
            callId: toolCall.id,
