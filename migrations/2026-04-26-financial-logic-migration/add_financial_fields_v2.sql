-- Enhance financials table with new columns for detailed cost tracking (keeping existing for backward compatibility)
...
-- Add detailed cost columns to order_items table for per-item tracking
ALTER TABLE order_items
  ADD COLUMN IF NOT EXISTS fob_cost DECIMAL(10, 2) DEFAULT 0.00,
  ADD COLUMN IF NOT EXISTS supplier_freight DECIMAL(10, 2) DEFAULT 0.00,
  ADD COLUMN IF NOT EXISTS customs_nationalization DECIMAL(10, 2) DEFAULT 0.00;

-- Create or update view for order financial summary with proper aggregation from order_items
...
SELECT
    ...
    o.status,
    -- Financial summary from order_items (preferred source for detailed tracking)
...
-- Create a function to calculate price from cost and margin (for use in triggers or applications)
...
-- Create a function to calculate margin from cost and price
...
-- Create a function to calculate price with VAT (hardcoded 15%)
